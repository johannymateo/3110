...
const Database Database::operator= (const Database &otherRight)
{
    delete [] this->phoneArr;
    this->items = otherRight.items;
    this->dbSize = otherRight.dbSize;
    this->phoneArr = new Phone[this->dbSize];
    for (int i = 0; i < this->dbSize; i++)
        this->phoneArr[i] = otherRight.phoneArr[i];
    return *this;
}
...
void Database::sizePrompt()
{
    ...
    cerr << "What is the minimum number of items you want to add: ";
    cin >> tempSize;
    ...
    dbSize = tempSize;        // number of items they want to add
...
void Database::addItem()
{
    ...
    while ( (items < dbSize) && (!dupe) ) {

        // if the arr is almost full, call the grow fn
        if ( items <= (dbSize/2) )
            growPhoneArr();
...
void Database::growPhoneArr()
{
    // create new array with double size
    Phone *phoneArrGrown = new Phone[this->dbSize*2];

    // copy everything from current array to new array
    for (int i = 0; i < this->dbSize; i++)
        *(phoneArrGrown + i) = this->phoneArr[i];

    // old array has been copied over, delete it
    delete [] this->phoneArr;

    // set phoneArr ptr to point to new array and change dbSize
    this->phoneArr = phoneArrGrown;
    phoneArrGrown = NULL;		// phoneArr is now pointing here, nullify this
    this->dbSize *= 2;			// new DB size
    return;
}
